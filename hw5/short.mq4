...
void OnTimer()
{
    ...
    if (currentHour >= 15 && currentHour < 17)
    {
        ...
        if (executedVolume < FirstWindowVolume)
        {
            ...
            int ticket = OrderSend(SymbolToTrade, OP_SELL, lots, Bid, 3, 0, 0, "USDCAD First Window", 0, 0, clrGreen);
            ...
            TradesExecuted1++;
            AvgExecutionPrice1 = ((AvgExecutionPrice1 * (TradesExecuted1 - 1)) + Bid) / TradesExecuted1;
...
if (currentHour >= 19 && currentHour < 22)
{
    ...
    if (executedVolume < SecondWindowVolume)
    {
        ...
        int ticket = OrderSend(SymbolToTrade, orderType, lots, Bid, 3, 0, 0, "USDCAD Second Window", 0, 0, clrBlue);
        ...
        else
        {
            Print("Order opened successfully: ", ticket);
            TradesExecuted2++;
            AvgExecutionPrice2 = ((AvgExecutionPrice2 * (TradesExecuted2 - 1)) + Bid) / TradesExecuted2;
            lastTradeTime = currentTime;
        }
    }
...
if (currentHour >= 23 || currentHour < 1)
{
    ...
    if (executedVolume < updatedThirdWindowVolume)
    {
        ...
        int ticket = OrderSend(SymbolToTrade, orderType, lots, Bid, 3, 0, 0, "USDCAD Third Window", 0, 0, clrYellow);
        ...
        else
        {
            Print("Order opened successfully: ", ticket);
            TradesExecuted3++;
            AvgExecutionPrice3 = ((AvgExecutionPrice3 * (TradesExecuted3 - 1)) + Bid) / TradesExecuted3;
            lastTradeTime = currentTime;
        }
    }
...
if (currentHour >= 3 && currentHour < 6)
{
    ...
    if (executedVolume < updatedFourthWindowVolume)
    {
        ...
        int ticket = OrderSend(SymbolToTrade, orderType, lots, Bid, 3, 0, 0, "USDCAD Fourth Window", 0, 0, clrPurple);
        ...
        if (ticket < 0)
        {
         ...
         else
         {
            Print("Order opened successfully: ", ticket);
            TradesExecuted4++;
            AvgExecutionPrice4 = ((AvgExecutionPrice4 * (TradesExecuted4 - 1)) + Bid) / TradesExecuted4;
            lastTradeTime = currentTime;
         }
}
